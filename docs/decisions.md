...
12. **Fakes ship in `app/adapters/` and are this pass's real adapters.**
    `FakeSource` (dict-backed) and `FakeWarehouse` (in-memory, write-only) are wired by `build_adapters()`. They are not test-only utilities: they are the production adapters for this pass, and the tests reuse them via conftest fixtures (`make_fake_source`, `fake_warehouse`) rather than maintaining duplicate test doubles. Only wiring-specific test collaborators (dummy mappings, recording/failing builders, call-counting adapter subclasses) live in `tests/`.
